"""Plot Muon's workload profile using the cached standard-target leaderboard."""
...
OUTPUT = HERE / 'out'
MUONS = ['Muon (PyTorch)', 'Muon (JAX)']
COLORS = ['#176B96', '#B65A27']
MARKERS = ['o', 'D']
WORKLOADS = [
...
def load_verified_data(source=SOURCE, config=None):
  """Validate cached medians against study summaries, keyed by submission."""
  ...
  if not times.index.is_unique or not set(MUONS).issubset(times.index):
    raise ValueError(
...
def plot(times, normalized, best):
  ...
  handles = [
    Line2D(
      [], [], marker=m, color=c, linestyle='none', markersize=7, label=name
    )
    for name, c, m in zip(MUONS, COLORS, MARKERS, strict=True)
  ] + [
    Line2D(
      [],
      [],
  ...
  ax = fig.add_axes([0.222, 0.12, 0.588, 0.73])
  ratios = fig.add_axes([0.834, 0.12, 0.146, 0.73])
  ...
  for x, text, color in zip([0.24, 0.76], ['PyTorch', 'JAX'], COLORS):
    ratios.text(
      x,
      1.045,
      text,
  ...
  other_names = times.index.difference(MUONS)
...
def main():
  ...
  times, normalized, best, audit = load_verified_data(args.source)
  ...
  plt.close(fig)
  print(
    f'Verified {len(audit)} time-to-target entries against study summaries.'
  )
  print(*paths, sep='\n')
