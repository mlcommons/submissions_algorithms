...
HPARAMS = {
  ...
  "warmup_factor": 0.02,
  "alpha": 8.5363279656321,
  "momentum_warmup_fraction": 0.5963481000816313,
  "grad_clip": 0.5,
}
...
def init_optimizer_state(workload: spec.Workload,
  ...
  # Ratio-based momentum warmup dynamically scaled by workload.step_hint
  if hasattr(HPARAMS, 'momentum_warmup_fraction'):
    momentum_warmup_steps = int(HPARAMS.momentum_warmup_fraction * workload.step_hint)
    beta3_warmup = momentum_warmup_steps
    alpha_warmup = momentum_warmup_steps
  else:
    beta3_warmup = getattr(HPARAMS, 'beta3_warmup', None)
    alpha_warmup = getattr(HPARAMS, 'alpha_warmup', None)
